...
public class UserController {
    ...
    @GetMapping("/getUsersByPaging")
    public Result getUsersByPaging(@RequestParam("page") Integer page) {
        Result result = Result.success();
        try {
            result.add("users", userService.getUsersByPaging(page));
        } catch (DataAccessException e) {
            logger.error(e.getMessage());
            return Result.error(Result.DATABASE_ERROR);
        }
    ...
    public Result getAllUsers() {
        return Result.success().add("users", userService.getAllUsers());
    }
